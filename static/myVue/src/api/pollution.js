...
export const download = async (ids) => {
  const url = new URL('/pollution/download', import.meta.env.VITE_API_BASE || 'http://localhost:8085')
...
export const taskResult = async (taskId) => {
  const url = `${import.meta.env.VITE_API_BASE || 'http://localhost:8085'}/pollution/async/result/${taskId}`
